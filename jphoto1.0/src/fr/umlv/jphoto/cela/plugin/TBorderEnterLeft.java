 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class TBorderEnterLeft implements ImageObserver,Transition{
	...
	public void init(TransitionConfig config) {
		...
		buffer = new BufferedImage(dimension.width, dimension.height, BufferedImage.TYPE_INT_RGB); 
			//Toolkit.getDefaultToolkit().createImage(new byte[dimension.height*dimension.width]);
		gBuffer  = (Graphics2D)buffer.getGraphics();//  getGraphics();
	...
	public void draw(long time, Graphics2D graphics) {
		...
		int startSrcX;
		startSrcX = (int)((time*dimension.width)/(Duration+1));
		
		gBuffer  = (Graphics2D)buffer.getGraphics();
		gBuffer.setBackground(new Color(0,0,0));
		gBuffer.clearRect(0,0,dimension.width, dimension.height);
		gBuffer.drawImage(dst,startSrcX - dimension.width +inset2x,inset2y, null);
		gBuffer.drawImage(src,startSrcX+inset1x,inset1y,null);
		gBuffer.dispose();
