...
import fr.umlv.jphoto.cela.data.TSlide;
 ...
 * @see JTable
 * @see fr.umlv.jphoto.cela.ui.JSlideTable
 */
public class SlideShowAdapterTableColumnModel implements TableColumnModel,
...
//
    public SlideShow  getSlideShow()
    {
    	...
    	{
    		slide = (Slide)getColumn(index).getIdentifier();
    		slideshow.add(index,slide);
    	}
    	return  slideshow;
    }
    
    public SlideShowAdapterTableColumnModel(JphotoAdaptaterListModel model) {
    	this();
    	this.model = model;
    	...
    	model.addListDataListener(new ListDataListener(){
			...
			public void intervalAdded(ListDataEvent e) {
				...
				for(int index=e.getIndex0();index<=e.getIndex1();index++)
					{
						Picture picture = (Picture)((ListModel)e.getSource()).getElementAt(index);
						TableColumn column = new TableColumn();
						Slide slide;
						if(picture.getPlugin()!=null)
						{
							slide = new TSlide(5000, picture.getPlugin().createTransition(), picture.getPictureIcon());
						}
						else
						{
							slide = new PSlide(5000, new ImageIcon(picture.getPicturePath()).getImage());
						}
						slide.addPropertyChangeListener(new PropertyChangeListener()
							{

								public void propertyChange(PropertyChangeEvent evt) {
									if(evt.getPropertyName()=="duration")
									{
										
										getColumn(getColumnIndex(evt.getSource())).setWidth(
												((Long)(evt.getNewValue())).intValue()/10);
										getColumn(getColumnIndex(evt.getSource())).setPreferredWidth(
												((Long)(evt.getNewValue())).intValue()/10);
									}
								}
					...
					column.setPreferredWidth((int)slide.getDuration()/10);
					column.setIdentifier(slide);
					column.addPropertyChangeListener(new PropertyChangeListener()
							{

								public void propertyChange(PropertyChangeEvent evt) {
									if(evt.getPropertyName()=="width")
									{
										((Slide)((TableColumn)evt.getSource()).getIdentifier())
										.setDuration(((Integer)evt.getNewValue()).longValue()*10	);
									}
								}
     ...
     */
    public SlideShowAdapterTableColumnModel() {
	super();
...
//
     ...
     * @see	#removeColumn
     */
    private void addColumn(int index, TableColumn aColumn) {
	
  /*  if(getColumnCount()==1)
    {
    	if(getColumn(0).getIdentifier() instanceof Integer)
    	{
	...
	tableColumns.add(index, aColumn);
  ...
  /*  if(getColumnCount()==1)
    ...
    public void addColumn(Picture picture)
    {
	...
	model.moveInterval(columnIndex, newIndex);
